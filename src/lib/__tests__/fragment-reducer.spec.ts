...
import {
  expectedFragmentsReduced as efr1,
  operationQuery as op1
} from "./common1.fixtures";
import {
  expectedFragmentsReduced as efr2,
  operationQuery as op2
} from "./common2.fixtures";
...
describe("fragmentReducer(documentNode): operationNode", () => {
  it("returns null with no definitions", () => {
    ...
    expect(
      fragmentReducer(({ definitions: [] } as unknown) as DocumentNode)
    ).toBeNull();
  });
